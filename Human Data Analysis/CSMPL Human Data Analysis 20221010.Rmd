...
accplot<-qplot(model,acc,data=plotdata,fill=model,geom="blank",xlab="Preference Model",ylab="Simulation Accuracy")+geom_bar(stat="identity")+
  geom_errorbar(aes(ymin=lci,ymax=uci))+theme_gray(base_size=20)+theme(axis.text.x=element_text(angle=90,hjust=1),legend.position="none")

if(sf==1){
  ggsave("Figures/CAREER1 Simulation Accuracy.tiff",accplot,width=12,height=6,units="in",dpi=100)
}
